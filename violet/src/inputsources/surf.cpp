...
class Surf : public InputSource
{
    ...
    static bayesian_fusion::ConfusionMatrix initializeConfusionMatrix(ros::NodeHandle& local_nh_);
public:
    Surf();
    virtual void callback(const violet_msgs::DetectionInfo::ConstPtr &msg);
};

Surf::Surf() : InputSource("Surf"),
               local_nh("~"),
               min_volumetric_intersection(local_nh.param<double>("input_source_options/Surf/min_volumetric_intersection", 0.25)),
               confusion_matrix(initializeConfusionMatrix(local_nh))
{
    double sigma_defaults[9] = {0.03, 0, 0, 0, 0.02, 0, 0, 0, 0.03};
    std::vector<double> sigma_vec = local_nh.param<std::vector<double> >("input_source_options/Surf/location_pdf_sigma_matrix",
    ...
    std::vector<double> confidence_vec = local_nh.param<std::vector<double> >("input_source_options/Surf/confidence_confusion_matrix",
...
bayesian_fusion::ConfusionMatrix Surf::initializeConfusionMatrix(ros::NodeHandle& local_nh_)
{
    std::string file_name;
    file_name = local_nh_.param<std::string>("input_source_options/Surf/detection_confusion_matrix_file", "");
...
void Surf::callback(const violet_msgs::DetectionInfo::ConstPtr &msg)
{
    ...
    for(violet_msgs::DetectionInfo::_objects_type::const_iterator object_it = msg->objects.begin(); object_it != msg->objects.end(); ++object_it) {
        ...
        try
        {
            if(!tf_listener.waitForTransform(FIXED_FRAME, msg->header.frame_id, ros::Time(0), ros::Duration(1))) {
                continue;
            }

            tf_listener.transformPoint(FIXED_FRAME, camera_location, world_location);
        }
        catch (tf::TransformException ex)
        {
            ROS_WARN("Transform Exception in source %s: %s to %s, in %s. Cause is: %s", _src_name.c_str(), msg->header.frame_id.c_str(), FIXED_FRAME.c_str(), __PRETTY_FUNCTION__, ex.what());
            continue;
        }
