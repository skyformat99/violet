...
bool FovPredicate::check(const ObjectInfo &object)
{
    ...
    tf::Stamped<tf::Pose> map_pose(tf::Pose(object.orientation(), object.location() ), object.lastDetectionTime("LineMod"), FIXED_FRAME);
    tf::Stamped<tf::Pose> cam_pose;

    try
    {
        listener->waitForTransform(object.fovFrame(), FIXED_FRAME, ros::Time(0), ros::Duration(0.1));
        listener->transformPose(object.fovFrame(), ros::Time(0), map_pose, FIXED_FRAME, cam_pose);
    }
    ...
    return (point.z() >= min_distance_from_camera &&
            (std::abs( std::atan2( point.x(), point.z() ) ) <= max_horizontal_angle)  &&
            (std::abs( std::atan2( point.y(), point.z() ) ) <= max_vertical_angle));
}
