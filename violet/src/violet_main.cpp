...
VioletMain::VioletMain() : local_node_handle("~")
{
    ...
    FIXED_FRAME = local_node_handle.param<std::string>("fixed_frame", "/map");
...
{
    ADD_REMOVE_PAUSED = !ADD_REMOVE_PAUSED; //toggle pause
    return true;
}
...
{
    ros::init(argc, argv, "violet_node");
    ros::NodeHandle n;
