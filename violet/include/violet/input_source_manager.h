...
{
    static InputSource* constructInputSource()
    {
        return new InputSourceType;
    }
public:
    InputSourceFactory(const char* source_name)
    {
        InputSourceManager::registerSource(source_name, boost::function<InputSource* ()>(&InputSourceFactory<InputSourceType>::constructInputSource));
    }
};
